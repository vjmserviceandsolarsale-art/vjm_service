document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleUserInput() {
    ...
    if (step === 0) {
      ...
      setTimeout(
        () => addBot("📞 Please share your primary phone number."),
        600
      );
      step++;
    } else if (step === 1) {
      ...
      // saveToSheet(userData);
      setTimeout(
        () => addBot("📞 Please provide an alternate contact number."),
        600
      );
      step++;
    } else if (step === 2) {
  ...
  function finishChat() {
    ...
    setTimeout(() => {
      addBot(
        `<a href="${whatsappURL}" target="_blank" class="call-link">📲 Click here to chat on WhatsApp</a>`
      );
    }, 1000);
  }
  ...
  if (whatsappBtn && form) {
    whatsappBtn.addEventListener("click", (e) => {
      ...
      const whatsappURL = `https://wa.me/916380886238?text=${encodeURIComponent(
        message
      )}`;
  ...
  if (leftModalBtn)
    leftModalBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      if (current > 0) {
        current--;
        modalImg.src = galleryImages[current];
  ...
  if (rightModalBtn)
    rightModalBtn.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.key === "ArrowRight" && current < galleryImages.length - 1)
      rightModalBtn && rightModalBtn.click();
    if (e.key === "ArrowLeft" && current > 0)
      leftModalBtn && leftModalBtn.click();
